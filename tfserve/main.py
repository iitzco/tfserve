...
JSON HANDLER
  ...
  Inputs are submitted to the root path '/' using HTTP POST and
  contain a JSON encoded map of input tensors to input
  values. Input values must comply with the expected tensors
  shape. If batch mode is enabled, multiple input values must be
  submitted in an array.

  A response body is a JSON encoded map of output tensors to output
  values generated by the model for the given the inputs. If batch
  mode is enabled, output values are returned in an array.

CUSTOM HANDLERS

  A Python module name may be specified with --handler. The module
